...
export default function App() {
  ...
  useEffect(() => {
    if (!loaded) return;
    (async () => {
      try { await window.storage.set(STORAGE_KEY, JSON.stringify({ baby, visits, saved, loc, recentLocs })); }
      catch (e) { console.error("Save failed", e); }
    })();
  }, [baby, visits, saved, loc, recentLocs, loaded]);
            ...
            <div className="pad">
              ...
              {rated.map((v) => (
                <div key={v.id} className="mem">
                  ...
                  <div className="mem-venue">{v.emoji} {v.name}{v.loc ? <span className="mem-loc"> · near {v.loc}</span> : null}</div>
