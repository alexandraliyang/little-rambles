...
/* ================================================================
   Little Rambles v0.2
   NEW vs v0.1:
   1. Developmental engine now runs on the affordance model from
      developmental-map-v1.json (age band -> hungry skills ->
      affordances -> venues). Every recommendation is explainable.
   2. Availability pre-processing: typical hours + season + daylight
      are checked BEFORE ranking. Closed places never lead. Labels
      are honest about confidence ("typical hours — confirm").
   ================================================================ */
...
const mapsUrl = (q) => "https://www.google.com/maps/search/?api=1&query=" + encodeURIComponent(q + " near me");
...
export default function App() {
  ...
  useEffect(() => {
    if (!loaded) return;
    (async () => {
      try { await window.storage.set(STORAGE_KEY, JSON.stringify({ baby, visits, saved })); }
  ...
  function score(idea, avail) {
    ...
    if (rated.slice(0, 4).some((v) => v.ideaId === idea.id)) s -= 3; // anti-samey
  ...
  }, [months, rated, lovedCats, pausedCats, retryIdeas, tab]);
  ...
  const goOut = (idea) => {
    setVisits((v) => [{ id: Date.now(), ideaId: idea.id, name: idea.name, cat: idea.cat, emoji: idea.emoji, ts: Date.now(), rating: null, note: "" }, ...v]);
            ...
            <div className="pad">
              ...
              {hero ? (
                <div className="card hero-card">
                  ...
                  <div className="btn-row">
                    <a className="primary link-btn" href={mapsUrl(hero.idea.mapsQuery)} target="_blank" rel="noreferrer" onClick={() => goOut(hero.idea)}>Let's go · open Maps</a>
              ...
              {openRanked.slice(1, 4).map(({ idea, avail, fit }) => (
                <IdeaCard key={idea.id} idea={idea} fit={fit} avail={avail} affs={matchedAffordances(idea)} onGo={goOut} onSave={toggleSave} savedList={saved} compact />
              ))}
          ...
          {tab === "explore" && (
            <div className="pad">
              ...
              {filtered.map(({ idea, avail, fit }) => (
                <IdeaCard key={idea.id} idea={idea} fit={fit} avail={avail} affs={matchedAffordances(idea)} onGo={goOut} onSave={toggleSave} savedList={saved} />
              ))}
...
function IdeaCard({ idea, fit, avail, affs, onGo, onSave, savedList, compact }) {
